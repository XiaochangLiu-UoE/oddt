...
def hbond_acceptor_donor(mol1, mol2, cutoff=3.5, tolerance=30):
    ...
    if len(a) > 0 and len(d) > 0:
        ...
        angle2 = angle(a['coords'][:, np.newaxis, :],
                       d['coords'][:, np.newaxis, :],
                       d['neighbors'])
        strict = ((np.nan_to_num(angle1) > (np.take(BASE_ANGLES, a['hybridization']) - tolerance)[:, np.newaxis]) &
                  (np.nan_to_num(angle1) < (np.take(BASE_ANGLES, a['hybridization']) + tolerance)[:, np.newaxis]) &
                  ((np.nan_to_num(angle2) > (np.take(BASE_ANGLES, d['hybridization']) - tolerance)[:, np.newaxis]) &
                   (np.nan_to_num(angle2) < (np.take(BASE_ANGLES, d['hybridization']) + tolerance)[:, np.newaxis]))
                  ).any(axis=-1)
        return a, d, strict
    else:
...
def halogenbond_acceptor_halogen(mol1,
    ...
    if len(a) > 0 and len(h) > 0:
        ...
        strict = ((np.nan_to_num(angle1) > (np.take(BASE_ANGLES, a['hybridization']) - tolerance)[:, np.newaxis]) &
                  (np.nan_to_num(angle1) < (np.take(BASE_ANGLES, a['hybridization']) + tolerance)[:, np.newaxis]) &
                  (np.nan_to_num(angle2) > (np.take(BASE_ANGLES, h['hybridization']) - tolerance)[:, np.newaxis]) &
                  (np.nan_to_num(angle2) < (np.take(BASE_ANGLES, h['hybridization']) + tolerance)[:, np.newaxis])
                  ).any(axis=-1)
        return a, h, strict
    else:
